...
bool F2fsIostatLatencyFtraceEvent::operator==(const F2fsIostatLatencyFtraceEvent& other) const {
  return unknown_fields_ == other.unknown_fields_
   && d_rd_avg_ == other.d_rd_avg_
   && d_rd_cnt_ == other.d_rd_cnt_
   && d_rd_peak_ == other.d_rd_peak_
   && d_wr_as_avg_ == other.d_wr_as_avg_
   && d_wr_as_cnt_ == other.d_wr_as_cnt_
   && d_wr_as_peak_ == other.d_wr_as_peak_
   && d_wr_s_avg_ == other.d_wr_s_avg_
   && d_wr_s_cnt_ == other.d_wr_s_cnt_
   && d_wr_s_peak_ == other.d_wr_s_peak_
   && dev_ == other.dev_
   && m_rd_avg_ == other.m_rd_avg_
   && m_rd_cnt_ == other.m_rd_cnt_
   && m_rd_peak_ == other.m_rd_peak_
   && m_wr_as_avg_ == other.m_wr_as_avg_
   && m_wr_as_cnt_ == other.m_wr_as_cnt_
   && m_wr_as_peak_ == other.m_wr_as_peak_
   && m_wr_s_avg_ == other.m_wr_s_avg_
   && m_wr_s_cnt_ == other.m_wr_s_cnt_
   && m_wr_s_peak_ == other.m_wr_s_peak_
   && n_rd_avg_ == other.n_rd_avg_
   && n_rd_cnt_ == other.n_rd_cnt_
   && n_rd_peak_ == other.n_rd_peak_
   && n_wr_as_avg_ == other.n_wr_as_avg_
   && n_wr_as_cnt_ == other.n_wr_as_cnt_
   && n_wr_as_peak_ == other.n_wr_as_peak_
   && n_wr_s_avg_ == other.n_wr_s_avg_
   && n_wr_s_cnt_ == other.n_wr_s_cnt_
   && n_wr_s_peak_ == other.n_wr_s_peak_;
}
...
void F2fsIostatLatencyFtraceEvent::Serialize(::protozero::Message* msg) const {
  // Field 1: d_rd_avg
  if (_has_field_[1]) {
    msg->AppendVarInt(1, d_rd_avg_);
  }

  // Field 2: d_rd_cnt
  if (_has_field_[2]) {
    msg->AppendVarInt(2, d_rd_cnt_);
  }

  // Field 3: d_rd_peak
  if (_has_field_[3]) {
    msg->AppendVarInt(3, d_rd_peak_);
  }

  // Field 4: d_wr_as_avg
  if (_has_field_[4]) {
    msg->AppendVarInt(4, d_wr_as_avg_);
  }

  // Field 5: d_wr_as_cnt
  if (_has_field_[5]) {
    msg->AppendVarInt(5, d_wr_as_cnt_);
  }

  // Field 6: d_wr_as_peak
  if (_has_field_[6]) {
    msg->AppendVarInt(6, d_wr_as_peak_);
  }

  // Field 7: d_wr_s_avg
  if (_has_field_[7]) {
    msg->AppendVarInt(7, d_wr_s_avg_);
  }

  // Field 8: d_wr_s_cnt
  if (_has_field_[8]) {
    msg->AppendVarInt(8, d_wr_s_cnt_);
  }

  // Field 9: d_wr_s_peak
  if (_has_field_[9]) {
    msg->AppendVarInt(9, d_wr_s_peak_);
  }

  // Field 10: dev
  if (_has_field_[10]) {
    msg->AppendVarInt(10, dev_);
  }

  // Field 11: m_rd_avg
  if (_has_field_[11]) {
    msg->AppendVarInt(11, m_rd_avg_);
  }

  // Field 12: m_rd_cnt
  if (_has_field_[12]) {
    msg->AppendVarInt(12, m_rd_cnt_);
  }

  // Field 13: m_rd_peak
  if (_has_field_[13]) {
    msg->AppendVarInt(13, m_rd_peak_);
  }

  // Field 14: m_wr_as_avg
  if (_has_field_[14]) {
    msg->AppendVarInt(14, m_wr_as_avg_);
  }

  // Field 15: m_wr_as_cnt
  if (_has_field_[15]) {
    msg->AppendVarInt(15, m_wr_as_cnt_);
  }

  // Field 16: m_wr_as_peak
  if (_has_field_[16]) {
    msg->AppendVarInt(16, m_wr_as_peak_);
  }

  // Field 17: m_wr_s_avg
  if (_has_field_[17]) {
    msg->AppendVarInt(17, m_wr_s_avg_);
  }

  // Field 18: m_wr_s_cnt
  if (_has_field_[18]) {
    msg->AppendVarInt(18, m_wr_s_cnt_);
  }

  // Field 19: m_wr_s_peak
  if (_has_field_[19]) {
    msg->AppendVarInt(19, m_wr_s_peak_);
  }

  // Field 20: n_rd_avg
  if (_has_field_[20]) {
    msg->AppendVarInt(20, n_rd_avg_);
  }

  // Field 21: n_rd_cnt
  if (_has_field_[21]) {
    msg->AppendVarInt(21, n_rd_cnt_);
  }

  // Field 22: n_rd_peak
  if (_has_field_[22]) {
    msg->AppendVarInt(22, n_rd_peak_);
  }

  // Field 23: n_wr_as_avg
  if (_has_field_[23]) {
    msg->AppendVarInt(23, n_wr_as_avg_);
  }

  // Field 24: n_wr_as_cnt
  if (_has_field_[24]) {
    msg->AppendVarInt(24, n_wr_as_cnt_);
  }

  // Field 25: n_wr_as_peak
  if (_has_field_[25]) {
    msg->AppendVarInt(25, n_wr_as_peak_);
  }

  // Field 26: n_wr_s_avg
  if (_has_field_[26]) {
    msg->AppendVarInt(26, n_wr_s_avg_);
  }

  // Field 27: n_wr_s_cnt
  if (_has_field_[27]) {
    msg->AppendVarInt(27, n_wr_s_cnt_);
  }

  // Field 28: n_wr_s_peak
  if (_has_field_[28]) {
    msg->AppendVarInt(28, n_wr_s_peak_);
  }
...
bool F2fsIostatFtraceEvent::operator==(const F2fsIostatFtraceEvent& other) const {
  return unknown_fields_ == other.unknown_fields_
   && app_bio_ == other.app_bio_
   && app_brio_ == other.app_brio_
   && app_dio_ == other.app_dio_
   && app_drio_ == other.app_drio_
   && app_mio_ == other.app_mio_
   && app_mrio_ == other.app_mrio_
   && app_rio_ == other.app_rio_
   && app_wio_ == other.app_wio_
   && dev_ == other.dev_
   && fs_cdrio_ == other.fs_cdrio_
   && fs_cp_dio_ == other.fs_cp_dio_
   && fs_cp_mio_ == other.fs_cp_mio_
   && fs_cp_nio_ == other.fs_cp_nio_
   && fs_dio_ == other.fs_dio_
   && fs_discard_ == other.fs_discard_
   && fs_drio_ == other.fs_drio_
   && fs_gc_dio_ == other.fs_gc_dio_
   && fs_gc_nio_ == other.fs_gc_nio_
   && fs_gdrio_ == other.fs_gdrio_
   && fs_mio_ == other.fs_mio_
   && fs_mrio_ == other.fs_mrio_
   && fs_nio_ == other.fs_nio_
   && fs_nrio_ == other.fs_nrio_;
}
...
void F2fsIostatFtraceEvent::Serialize(::protozero::Message* msg) const {
  // Field 1: app_bio
  if (_has_field_[1]) {
    msg->AppendVarInt(1, app_bio_);
  }

  // Field 2: app_brio
  if (_has_field_[2]) {
    msg->AppendVarInt(2, app_brio_);
  }

  // Field 3: app_dio
  if (_has_field_[3]) {
    msg->AppendVarInt(3, app_dio_);
  }

  // Field 4: app_drio
  if (_has_field_[4]) {
    msg->AppendVarInt(4, app_drio_);
  }

  // Field 5: app_mio
  if (_has_field_[5]) {
    msg->AppendVarInt(5, app_mio_);
  }

  // Field 6: app_mrio
  if (_has_field_[6]) {
    msg->AppendVarInt(6, app_mrio_);
  }

  // Field 7: app_rio
  if (_has_field_[7]) {
    msg->AppendVarInt(7, app_rio_);
  }

  // Field 8: app_wio
  if (_has_field_[8]) {
    msg->AppendVarInt(8, app_wio_);
  }

  // Field 9: dev
  if (_has_field_[9]) {
    msg->AppendVarInt(9, dev_);
  }

  // Field 10: fs_cdrio
  if (_has_field_[10]) {
    msg->AppendVarInt(10, fs_cdrio_);
  }

  // Field 11: fs_cp_dio
  if (_has_field_[11]) {
    msg->AppendVarInt(11, fs_cp_dio_);
  }

  // Field 12: fs_cp_mio
  if (_has_field_[12]) {
    msg->AppendVarInt(12, fs_cp_mio_);
  }

  // Field 13: fs_cp_nio
  if (_has_field_[13]) {
    msg->AppendVarInt(13, fs_cp_nio_);
  }

  // Field 14: fs_dio
  if (_has_field_[14]) {
    msg->AppendVarInt(14, fs_dio_);
  }

  // Field 15: fs_discard
  if (_has_field_[15]) {
    msg->AppendVarInt(15, fs_discard_);
  }

  // Field 16: fs_drio
  if (_has_field_[16]) {
    msg->AppendVarInt(16, fs_drio_);
  }

  // Field 17: fs_gc_dio
  if (_has_field_[17]) {
    msg->AppendVarInt(17, fs_gc_dio_);
  }

  // Field 18: fs_gc_nio
  if (_has_field_[18]) {
    msg->AppendVarInt(18, fs_gc_nio_);
  }

  // Field 19: fs_gdrio
  if (_has_field_[19]) {
    msg->AppendVarInt(19, fs_gdrio_);
  }

  // Field 20: fs_mio
  if (_has_field_[20]) {
    msg->AppendVarInt(20, fs_mio_);
  }

  // Field 21: fs_mrio
  if (_has_field_[21]) {
    msg->AppendVarInt(21, fs_mrio_);
  }

  // Field 22: fs_nio
  if (_has_field_[22]) {
    msg->AppendVarInt(22, fs_nio_);
  }

  // Field 23: fs_nrio
  if (_has_field_[23]) {
    msg->AppendVarInt(23, fs_nrio_);
  }
...
void F2fsReadpageFtraceEvent::Serialize(::protozero::Message* msg) const {
  ...
  // Field 6: dir
  if (_has_field_[6]) {
    msg->AppendVarInt(6, dir_);
  }

  // Field 7: dirty
  if (_has_field_[7]) {
    msg->AppendVarInt(7, dirty_);
  }

  // Field 8: uptodate
  if (_has_field_[8]) {
    msg->AppendVarInt(8, uptodate_);
  }
